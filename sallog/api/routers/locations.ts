...
export const locationsRouter = router({
  record: protectedProcedure.input(z.object({ freightId: z.number(), lat: z.number(), lng: z.number() })).mutation(async ({ input, ctx }) => {
    const [driver] = await db.select().from(drivers).where(eq(drivers.userId, ctx.user.id));
    if (!driver) throw new TRPCError({ code: 'FORBIDDEN' });
  ...
  latestByFreight: protectedProcedure.input(z.object({ freightId: z.number() })).query(async ({ input, ctx }) => {
    await assertAccess(ctx, input.freightId);
    const [row] = await db.select().from(driverLocations).where(eq(driverLocations.freightId, input.freightId)).orderBy(desc(driverLocations.recordedAt)).limit(1);
    return row ?? null;
  }),

  historyByFreight: adminProcedure.input(z.object({ freightId: z.number(), limit: z.number().int().min(1).max(200).optional() })).query(async ({ input }) => {
    return db.select().from(driverLocations).where(eq(driverLocations.freightId, input.freightId)).orderBy(desc(driverLocations.recordedAt)).limit(input.limit ?? 50);
  }),
});
