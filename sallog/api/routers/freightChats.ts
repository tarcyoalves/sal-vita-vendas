...
export const freightChatsRouter = router({
  list: protectedProcedure.input(z.object({ freightId: z.number() })).query(async ({ input, ctx }) => {
    await assertAccess(ctx, input.freightId);
    return db.select().from(freightChats).where(eq(freightChats.freightId, input.freightId)).orderBy(asc(freightChats.createdAt));
  }),

  send: protectedProcedure.input(z.object({ freightId: z.number(), content: z.string().min(1).max(2000) })).mutation(async ({ input, ctx }) => {
    const senderRole = await assertAccess(ctx, input.freightId);
    const [row] = await db.insert(freightChats).values({ freightId: input.freightId, senderId: ctx.user.id, senderRole, content: input.content }).returning();
    return row;
  }),
});
