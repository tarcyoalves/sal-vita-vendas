    ...
    <div>
      ...
      <div className="g2" style={{ display:'grid', gridTemplateColumns:'1fr 300px', gap:16 }}>
          ...
          {recent.length === 0 ? (
            ...
            return (
              <div key={f.id} onClick={() => nav({ name:'freight-detail', id:f.id })}
                style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 18px', borderBottom:'1px solid var(--surface-3)', cursor:'pointer', transition:'background 0.1s' }}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <div className="card">
            <div style={{ fontFamily:"'Barlow Semi Condensed',sans-serif", fontSize:13, fontWeight:700, letterSpacing:0.8, color:'var(--text-3)', textTransform:'uppercase', marginBottom:14 }}>Ações Rápidas</div>
          ...
          {pending > 0 && (
            <div className="alert alert-warning" style={{ cursor:'pointer', marginBottom:0 }} onClick={() => nav({ name:'drivers' })}>
              ⚠️ {pending} motorista{pending>1?'s':''} aguardando
            </div>
          )}
        </div>
      </div>
