...
export default function AdminDashboard() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-6">
      ...
      {/* Free Plan Monitor — Neon + Email */}
      {freePlanData && (
        <Card className={`border-2 ${
          freePlanData.neon.status === 'critical' ? 'border-red-400 bg-red-50/60'
            : freePlanData.neon.status === 'warning' ? 'border-amber-400 bg-amber-50/60'
            : 'border-blue-200 bg-blue-50/30'
        }`}>
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-base">
              <Database size={18} className="text-blue-800" />
              Monitor de Limites — Planos Gratuitos
              {(freePlanData.neon.status !== 'ok' || freePlanData.resend.status !== 'ok' || freePlanData.brevo.status !== 'ok') && (
                <AlertTriangle size={16} className={freePlanData.neon.status === 'critical' ? 'text-red-500 animate-pulse' : 'text-amber-500'} />
              )}
            </CardTitle>
          </CardHeader>
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                    freePlanData.neon.status === 'critical' ? 'bg-red-200 text-red-800'
                      : freePlanData.neon.status === 'warning' ? 'bg-amber-200 text-amber-800'
                ...
                <div className="space-y-1">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                    {freePlanData.neon.tables.slice(0, 6).map((t: { name: string; sizePretty: string; sizeBytes: number; rows: number }) => (
                      <div key={t.name} className="flex items-center justify-between rounded-lg bg-white/70 border border-slate-200 px-2.5 py-1.5">
                        <span className="text-xs text-slate-700 font-medium truncate">{t.name}</span>
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <span className="text-[10px] text-slate-400">{t.rows.toLocaleString('pt-BR')} linhas</span>
                          <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${
                            t.sizeBytes > 50 * 1024 * 1024 ? 'bg-red-100 text-red-700'
                              : t.sizeBytes > 10 * 1024 * 1024 ? 'bg-amber-100 text-amber-700'
              ...
              {freePlanData.resend.accounts > 0 && (
                <div className="rounded-xl border p-3 space-y-2 bg-white">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-slate-700">Resend</span>
                    <span className="text-[10px] text-slate-400 ml-auto">{freePlanData.resend.accounts} conta(s)</span>
                  </div>
                  <div>
                    <div className="flex justify-between text-[11px] text-slate-500 mb-0.5">
                      <span>Hoje: {freePlanData.resend.dailyUsed} / {freePlanData.resend.dailyLimit}</span>
                      <span>{freePlanData.resend.dailyPercent}%</span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-slate-100 overflow-hidden">
                      <div className={`h-full rounded-full ${freePlanData.resend.dailyPercent > 90 ? 'bg-red-500' : freePlanData.resend.dailyPercent > 70 ? 'bg-amber-500' : 'bg-violet-500'}`}
                        style={{ width: `${Math.min(100, freePlanData.resend.dailyPercent)}%` }} />
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between text-[11px] text-slate-500 mb-0.5">
                      <span>Mês: {freePlanData.resend.monthlyUsed.toLocaleString('pt-BR')} / {freePlanData.resend.monthlyLimit.toLocaleString('pt-BR')}</span>
                      <span>{freePlanData.resend.monthlyPercent}%</span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-slate-100 overflow-hidden">
                      <div className={`h-full rounded-full ${freePlanData.resend.monthlyPercent > 90 ? 'bg-red-500' : freePlanData.resend.monthlyPercent > 70 ? 'bg-amber-500' : 'bg-violet-500'}`}
              ...
              {freePlanData.brevo.accounts > 0 && (
                <div className="rounded-xl border p-3 space-y-2 bg-white">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-slate-700">Brevo</span>
                    <span className="text-[10px] text-slate-400 ml-auto">{freePlanData.brevo.accounts} conta(s)</span>
                  </div>
                  <div>
                    <div className="flex justify-between text-[11px] text-slate-500 mb-0.5">
                      <span>Hoje: {freePlanData.brevo.dailyUsed} / {freePlanData.brevo.dailyLimit}</span>
                      <span>{freePlanData.brevo.dailyPercent}%</span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-slate-100 overflow-hidden">
                      <div className={`h-full rounded-full ${freePlanData.brevo.dailyPercent > 90 ? 'bg-red-500' : freePlanData.brevo.dailyPercent > 70 ? 'bg-amber-500' : 'bg-emerald-500'}`}
                        style={{ width: `${Math.min(100, freePlanData.brevo.dailyPercent)}%` }} />
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between text-[11px] text-slate-500 mb-0.5">
                      <span>Mês: {freePlanData.brevo.monthlyUsed.toLocaleString('pt-BR')} / {freePlanData.brevo.monthlyLimit.toLocaleString('pt-BR')}</span>
                      <span>{freePlanData.brevo.monthlyPercent}%</span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-slate-100 overflow-hidden">
                      <div className={`h-full rounded-full ${freePlanData.brevo.monthlyPercent > 90 ? 'bg-red-500' : freePlanData.brevo.monthlyPercent > 70 ? 'bg-amber-500' : 'bg-emerald-500'}`}
